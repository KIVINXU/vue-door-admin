    ...
    <el-row>
      <el-col>
        <el-table :data="list" ref="boxesTable"
          ...
          <el-table-column prop="usenum" :show-overflow-tooltip="true" label="已用箱数"></el-table-column>
          <el-table-column prop="freenum" :show-overflow-tooltip="true" label="未用箱数"></el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      //请求后台
      requestList(List) {
        fetchList('/cab/cabinetsHome', List).then(response => {
      ...
      handleFocus() {
        if (this.searchOption.length === 0) {
          fetchSearchOption('/cab/cabinetsHome', {method: 'FieldQuery'})
            .then(response => {
              const data = response.data;
              if (data.msg && data.msg !== '') {
                this.$message({
