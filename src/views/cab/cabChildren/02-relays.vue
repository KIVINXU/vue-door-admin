    ...
    <el-row>
      <el-col>
        <el-table :data="list" border
          ...
          <el-table-column prop="model" :show-overflow-tooltip="true" label="控制器型号"></el-table-column>
          <el-table-column prop="relay" width="100" :show-overflow-tooltip="true" label="控制器编号"></el-table-column>
          <el-table-column prop="portnum" width="100" :show-overflow-tooltip="true" label="端口数"></el-table-column>
      ...
      <el-form ref="dataForm"
        ...
        <el-row>
          <el-col :sm="24" :md="24">
            ...
            <el-form-item label="控制器型号" prop="model">
            ...
            <el-form-item label="端口数" prop="portnum">
            <el-input v-model.trim="temp.portnum" :maxlength="2"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
...
<script>
  ...
  export default {
    data() {
      ...
      //端口数限制
      const checkPortnum = (rule, value, callback) => {
        if (!validateNum(value)) {
          callback(new Error('只能输入纯数字'));
        } else if (value < 1 || value > 32) {
          callback(new Error('端口数限制1-32,默认16'))
        } else {
          callback();
        }
      }
      return {
        ...
        rules: {
          ...
          portnum: [
            ...
            {validator: checkPortnum, trigger: 'change'}]
        }
      }
    },
    created () {
    ...
    methods: {
      ...
      //请求后台
      requestList (List){
        fetchList('/cab/eqpRelays', List).then( response => {
...
//            });
      ...
      handleFocus() {
        if(this.searchOption.length === 0) {
          fetchSearchOption('/cab/eqpRelays',{method: 'FieldQuery'})
            .then(response => {
              const data = response.data;
              if(data.msg && data.msg !== ''){
                this.$message({
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            delete temp.seqid;
            SubmitTable('/cab/eqpRelays', temp).then(response => {
              const data = response.data;
              if(data.msg && data.msg !== ''){
                this.$message({
                  showClose: true,
      ...
      rowDelete(index, row) {
        ...
        SubmitTable('/cab/eqpRelays', deleteData).then(response => {
          const data = response.data;
          if(data.msg && data.msg !== ''){
            this.$message({
              showClose: true,
