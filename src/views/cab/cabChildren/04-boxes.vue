    ...
    <el-row>
      <el-col>
        <el-table :data="list" border
          ...
          <el-table-column prop="relay" :show-overflow-tooltip="true" label="控制器编号"></el-table-column>
          <el-table-column prop="port" :show-overflow-tooltip="true" label="端口编号"></el-table-column>
          ...
          <el-table-column prop="flag" :show-overflow-tooltip="true" label="  存储状态"></el-table-column>
      ...
      <el-form ref="dataForm"
        ...
        <el-row>
          <el-col :sm="24" :md="12">
            ...
            <el-form-item label="控制器编号" prop="relay">
              <el-input v-model.trim="temp.relay" :maxlength="3"></el-input>
            </el-form-item>
          ...
          <el-col :sm="24" :md="12">
            <el-form-item label="端口编号" prop="port">
              <el-input v-model.trim="temp.port" :maxlength="2"></el-input>
            </el-form-item>
            <el-form-item label="箱子编号" prop="box">
              <el-input v-model.trim="temp.box" :maxlength="2"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
...
<script>
  ...
  export default {
    data() {
      //控制器限制
      const checkRelay = (rule, value, callback) => {
        if (!validateNum(value)) {
          callback(new Error('只能输入纯数字'));
        } else if (value < 10 || value > 254) {
          callback(new Error('控制器限制10-254,默认254'))
        } else {
          callback();
        }
      };
      ...
      const checkPort = (rule, value, callback) => {
        if (!validateNum(value)) {
          ...
          callback(new Error('端口限制1-32,默认16'))
        } else {
          callback();
        }
      };
      ...
      const checkBox = (rule, value, callback) => {
        if (!validateNum(value)) {
          ...
          callback(new Error('箱子数限制1-32,默认1'))
        } else {
          callback();
        }
      };
      return {
        ...
        rules: {
          ...
          relay: [{required: true, message: '控制器地址不能为空', trigger: 'blur'},
            {validator: checkRelay, trigger: 'change'}],
          port: [{required: true, message: '控制器端口不能为空', trigger: 'blur'},
            {validator: checkPort, trigger: 'change'}],
          cabinet: [{required: true, message: '柜子编号不能为空', trigger: 'blur'},
            {validator: checkCabinet, trigger: 'change'}],
          box: [{required: true, message: '箱子编号不能为空', trigger: 'blur'},
    ...
    methods: {
      ...
      //请求后台
      requestList (List){
        fetchList('/cab/eqpAssigns', List).then( response => {
...
//            });
      ...
      handleFocus() {
        if(this.searchOption.length === 0) {
          fetchSearchOption('/cab/eqpAssigns',{method: 'FieldQuery'})
            .then(response => {
              const data = response.data;
              if(data.msg && data.msg !== ''){
                this.$message({
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            delete temp.flag;
            delete temp.seqid;
            SubmitTable('/cab/eqpAssigns', temp).then(response => {
              const data = response.data;
              if(data.msg && data.msg !== ''){
                this.$message({
                  showClose: true,
      ...
      rowDelete(index, row) {
        ...
        SubmitTable('/cab/eqpAssigns', deleteData).then(response => {
          const data = response.data;
          if(data.msg && data.msg !== ''){
            this.$message({
              showClose: true,
